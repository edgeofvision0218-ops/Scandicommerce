...
interface ShopifyPlatformPageData {
  ...
  shopifyEmpires?: { title?: string; features?: Array<{ title?: string; description?: string; highlight?: string }> }
  ...
  successStories?: { title?: string; subtitle?: string; caseStudies?: Array<{ clientName?: string; heading?: string; description?: string; imageUrl?: string }> }
}

export default async function ShopifyPlatformPage({ params }: { params: Promise<{ lang: string; slug?: string }> }) {
  const { lang } = await params
  const language = getLanguageFromParams({ lang })
  const pageData = await client.fetch<ShopifyPlatformPageData>(shopifyPlatformPageQuery, getQueryParams({}, language), { next: { revalidate: 0 } })
