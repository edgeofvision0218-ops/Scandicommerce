...
export async function generateMetadata() {
  const alternates = getAlternateLanguagesForMetadata('shopify/shopify_POS')
...
export default async function ShopifyPOSPage({
  ...
  const pageData = await client
    .fetch<ShopifyPosInfoPageData | null>(shopifyPosInfoPageQuery, getQueryParams({}, language), {
      next: { revalidate: 0 },
    })
    .catch(() => null)
