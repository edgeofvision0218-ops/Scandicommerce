...
export default function Header({ settings }: HeaderProps) {
  ...
  return (
    <>
    <header
      className={`w-full sticky top-0 z-[10000] transition-all duration-300 ${isScrolled
        ? 'bg-white/75 backdrop-blur-sm shadow-header'
        : 'bg-white shadow-header'
        }`}
    >
      <nav className="container py-2 sm:py-0 mx-auto px-4 sm:px-6 lg:px-8 max-w-[100vw] overflow-visible">
        <div className="flex items-center justify-between h-12 narrow:h-14 sm:h-16 lg:h-20 gap-4 narrow:gap-2 min-h-0">
          ...
          <div className="flex items-center gap-0.5 narrow:gap-1.5 sm:gap-3 lg:gap-1.5 xl:gap-4 flex-shrink-0 min-w-0">
            ...
            <LocalizedLink
              ...
              className="hidden min-[426px]:inline-block bg-teal text-white px-2 py-1.5 narrow:px-3 narrow:py-2 sm:px-6 sm:py-2.5 font-semibold hover:bg-teal-dark transition-colors shadow-button text-xs narrow:text-sm sm:text-base flex-shrink-0"
            >
              {ctaButton.label}
            </LocalizedLink>
          </div>
    ...
    {mounted &&
      ...
      createPortal(
        <div
          className={`lg:hidden fixed left-0 right-0 bottom-0 top-12 narrow:top-14 sm:top-16 z-[9999] transition-opacity duration-300 ${
        ...
        >
          ...
          <aside
            className={`absolute left-0 top-0 bottom-0 w-[min(280px,85vw)] bg-white shadow-xl overflow-y-auto transition-transform duration-300 ease-out ${
